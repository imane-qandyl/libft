...
char	**ft_split(char const *s, char c)
{
	...
	split = ((malloc(sizeof(char *) * count))); // malloc() returns a void * pointer, but split is of type char **. The types don't match. 
	if (!split)
		return (NULL);
	i = 0;
	while (i < count)
	{
		...
		if (!split)
			free_mem(*split, i); // since split is an array of strings we have to free the previous strings if one of them failed
		while (*s && *s != c)
			s++;
		i++;
	}
